...
async def send_email_gmail(recipient_email: str, cart_name: str, cart_items: List[ItemInDB]) -> dict:
    ...
    try:
        ...
        result = await send_email_ses(recipient_email, subject, body_html)
        
        # Log errors for monitoring (since email is sent in background, errors won't be returned to user)
        if "error" in result:
            logger.error(
                f"Failed to send cart share email to {recipient_email} for cart '{cart_name}': {result['error']}"
            )
        else:
            logger.info(
                f"Cart share email sent successfully to {recipient_email} for cart '{cart_name}'"
            )
        
        return result
    except Exception as e:
        # Catch any unexpected exceptions (e.g., from SES client, network issues, or test mocks)
        # Log them and return error dict instead of raising (since this runs in background task)
        error_msg = f"Unexpected error sending cart share email: {str(e)}"
        logger.error(f"{error_msg} to {recipient_email} for cart '{cart_name}'")
